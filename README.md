...
支持资源贡献者（Contributor）将资源从草稿阶段提交至审核流程，在资源被拒绝后进行修改与重新提交，提供提交状态的管理，形成完整的资源提交与审核流程。首先，贡献者可以提交已完成的资源。系统在提交前会对必要信息（如标题、分类、描述等）进行校验，只有在所有必填字段填写完整的情况下才允许提交。成功后，系统将资源状态从“Draft”更新为“Pending Review”，并记录提交时间。其次，在资源被拒绝后，贡献者可以根据审核反馈对资源进行修改。系统允许用户编辑并保存更新，同时保留审核反馈信息，以支持资源内容的改进。接着，贡献者可以将修改后的资源重新提交审核。系统会将资源状态更新为“Pending Review”，并记录新的提交时间，同时保留历史反馈信息。最后，系统提供状态查看功能。贡献者可以查看资源的当前状态，并在资源被拒绝时查看审核反馈，从而及时进行修改和再次提交。
