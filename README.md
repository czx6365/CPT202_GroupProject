...
该模块主要用于实现用户身份验证和基本的访问控制，面向所有用户（包括未注册访客、注册用户和管理员）。系统支持用户注册，要求填写必要信息，并对邮箱唯一性和密码强度进行校验。注册成功后，用户可以通过邮箱和密码登录，系统会根据用户角色跳转到对应页面。同时，用户可以随时退出登录，避免在公共设备上产生安全风险。此外，系统还提供找回密码功能，用户可通过邮箱重置密码。为了提升安全性，系统设置了登录失败次数限制、账户临时锁定以及验证码等机制，以防止恶意攻击。
...
4.Submission and Resubmission 彭孙熙

支持资源贡献者（Contributor）将资源从草稿阶段提交至审核流程，并在资源被拒绝后进行修改与重新提交，同时提供提交状态管理，形成完整的资源提交与审核流程。首先，贡献者可以提交资源。系统在提交前会对必要信息（如标题、分类、描述等）进行校验，只有在必填字段完整时才允许提交。提交成功后，资源状态由“Draft”更新为“Pending Review”，并记录提交时间，以保证流程的规范性。在资源被拒绝后，可以根据审核反馈对资源进行修改。系统允许保存更新，以支持资源改进。接着，贡献者可以将修改后的资源重新提交审核。系统会将状态更新为“Pending Review”，并记录新的提交时间，同时保留历史反馈信息。最后，系统提供状态查看功能。贡献者可以查看资源状态，并在被拒绝时查看反馈，从而进行后续修改与提交。
...
此模块主要面向未登录的公众访客（public vistor），帮助他们方便地发现和访问平台上已发布的社区文化遗产资源。该模块支持访客浏览资源列表、通过关键词（keywords）搜索相关内容，并利用分类（category）或标签（tag）进行筛选，从而在不知道资源准确标题的情况下也能快速找到感兴趣的信息。同时，系统还支持分页查看，用户可以分页浏览搜索结果和筛选结果，可以进行翻页操作，并且也可以进入资源详情页了解更完整的背景、元数据和相关材料，提高公众获取和探索文化遗产内容的效率与体验。

7.Resource Comment Management 许亦多

此模块的主要功能就是用户的评论与互动。所有的评论功能都只对登录的用户展开。所有的用户（registered viewer）在登录后都具有发布评论，修改评论和删除评论的功能。用户可以对喜欢的评论进行点赞。资源的贡献者（contributor）在这些功能以外，还具有删除他人在自己资源页下发布的评论的功能，以此来维护评论区氛围。

8.Master Data Management 邱虹瑜

此模块的核心任务是由平台管理员（Platform Administrator）对资源分类（category）进行统一管理。首先，管理员可以对分类执行完整的增删改查操作，包括创建新分类、编辑已有分类的名称与描述、浏览和搜索分类列表，以及删除不再使用的分类（已被引用的分类会被系统阻止删除）。其次，为保证所有变更的可追溯性，模块还提供审计日志功能，自动记录每次操作的执行人、时间和变更内容，并支持筛选查询和导出文件，且仅限授权用户访问。
